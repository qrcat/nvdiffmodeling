...
def render_layer_vert_wise(
    ...
    # Compute KD and KS
    assert mesh.v_kd is not None and mesh.v_ks is not None
    kd, _ = interpolate(mesh.v_kd[None, ...], rast_out_s, mesh.t_pos_idx.int())
    ks, _ = interpolate(mesh.v_ks[None, ...], rast_out_s, mesh.t_pos_idx.int())
    
    ################################################################################
    # Shade
    ################################################################################

    color = shade_with_kd_ks(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, kd, ks, 
        view_pos, light_pos, light_power, mesh.material['bsdf'], min_roughness)
...
def render_mesh_vert_wise(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), [resolution*spp, resolution*spp]) as peeler:
        for _ in range(num_layers):
            rast, db = peeler.rasterize_next_layer()
            layers += [(render_layer_vert_wise(rast, db, mesh, view_pos, light_pos, light_power, resolution, min_roughness, spp, msaa), rast)]
...
def render_layer_face_wise(
    ...
    # Compute KD and KS
    assert mesh.f_kd is not None and mesh.f_ks is not None
    tri_i = torch.arange(mesh.f_kd.size(0), device=mesh.f_kd.device).unsqueeze(-1).repeat(1, 3).int()
    kd, _ = interpolate(mesh.f_kd[None, ...], rast_out_s, tri_i)
    ks, _ = interpolate(mesh.f_ks[None, ...], rast_out_s, tri_i)
...
def render_mesh_face_wise(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), [resolution*spp, resolution*spp]) as peeler:
        for _ in range(num_layers):
            rast, db = peeler.rasterize_next_layer()
            layers += [(render_layer_face_wise(rast, db, mesh, view_pos, light_pos, light_power, resolution, min_roughness, spp, msaa), rast)]
